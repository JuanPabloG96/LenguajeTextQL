...
void Lexer::createTokenList(char c)
{
    ...
    else if (state >= 1000)
    {
        ...
        state = 0;
        buffer.clear();
        // Repetir el proceso para el siguiente caracter para evitar perder el
        // token de simbolos
        createTokenList(c);
    }
    else
...
void Lexer::printTokenList()
{
    ...
    while (current)
    {
        std::cout << std::left << std::setw(5) << current->getId()
                  << std::setw(50) << current->getTokenType()
                  << current->getToken() << std::endl;
